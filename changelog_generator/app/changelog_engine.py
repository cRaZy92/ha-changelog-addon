...
def run_changelog_generation_selected(config: Config, commit_hashes: list[str]) -> GenerationResult:
    """Generate changelog from user-selected commits."""
    ...
    if elapsed is not None and elapsed < config.cooldown_seconds:
        ...
        return GenerationResult(
            success=False, changelog="",
            error=f"Please wait {remaining} seconds before generating again.",
            metadata=None,
        )

    # 2. Build changeset from selected commits
    try:
        changeset_result = git_reader.get_changeset_for_commits(
        ...
        return GenerationResult(success=False, changelog="", error=str(e), metadata=None)

    if changeset_result is None:
        return GenerationResult(
            success=False, changelog="", error="No changes in selected commits.", metadata=None,
        )
    ...
    if not ai_result.success:
        return GenerationResult(
            success=False, changelog="", error=f"OpenAI error: {ai_result.error}", metadata=None,
        )
    ...
    # 6. Save state — use head_commit from selected commits
    try:
    ...
    return GenerationResult(
        success=True, changelog=ai_result.changelog, error=None, metadata=metadata,
    )
